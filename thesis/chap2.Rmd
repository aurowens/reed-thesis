...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;As outlined in the 1984 textbook, *Classification and Regression Trees*, Brieman, Friedman, Olshen, and Stone described their method for creating, pruning, and testing regression trees. What follows is their basic algorithm for fitting regression trees:
...
As mentioned in the introduction, CART has the tendency to bias towards variables with the most possible splits and overfitting. There is little head paid to statistical significance or general statistical theory. *Conditional Inference Trees* are a method proposed by Horthon et al, 2006, that utilizes permutation theory to create and algorithm that is sensitive to these issues. 
...
To compare the two methods, lets compare the CART tree fitted on $D_2$ above with a conditional inference tree as implemented in the R package `partykit`. 

### Figure 10:

**CART and CTree fitted to $D_2$**

```{r fig10, message=FALSE, warning=FALSE, echo=FALSE,cache=TRUE}
library(partykit)

t2.2 <- ctree(y~., round(d2))
#t2.2$frame$yval <- round(t2.2$frame$yval)

par(mfrow=c(1,2))
plot(t2.2, col = c(thesis, rep(thesis, 3)))
draw.tree(t2, digits = 2, col = c(thesis, rep(thesis, 3)))
```
...
Where  $n$  is the number of observations in  $D_{2, test}^i$, $y_j \in D_{2, test}^i, \hat{y_j} \in  T^i(D_{2, test}^i)$ for $1 \leq j \leq n$ 

This produces two distributions of $MSE_{test}$, one for CART and one for CTree, conditional inference trees. 
...
for(i in 1:1000){
  ...
  tCt <- ctree(y~., d2[train,])
  tC <- tree(y~., d2[train,])
  testmseC[i] <- mean((d2[-train,]$y - predict(tC, d2[-train,]))^2)
  testmseCt[i] <- mean((d2[-train,]$y - predict(tCt, d2[-train,]))^2)
}

testmse <- data.frame(ConditionalInferenceTree = testmseCt, CART= testmseC)

testm <- melt(testmse)

ggplot(data = testm, aes(fill = variable, x = value)) + 
  geom_density(alpha = .5)+
  ggtitle("Comparison of the Simulated MSEtest Distributions of CTree and CART")+
  scale_fill_manual(values = thesis)+
...
While trees may be variable, this variablility appears consistant across the algorithms considered here. 
...
for (i in 1:100){
  train = sample(1000,666)
  b <- randomForest(y~., data = d2, subset = train, mtry = 5, ntree = 100)
  mse[i] <- mean((d2$y[-train] - predict(b, d2[-train,]))^2)
}

msebag <- data.frame(BaggedForest = mse, CTree = sample(testmseCt,100))

msebag <- melt(msebag)

ggplot(aes(x = value, fill = variable), data = msebag) + 
  geom_density(alpha = .5)+
  ggtitle("Comparison of the Simulated MSEtest Distributions of CTree and Bagged Forests")+
  scale_fill_manual(values = thesis)+
  guides(fill=guide_legend(title=NULL))
...
```{r fig13, echo=FALSE, message=FALSE,cache=TRUE}
...
mserf <- data.frame(BaggedForest = mse, CTree = sample(testmseCt, 100), RandomForest = mseRF)

mserf<- melt(mserf)

ggplot(aes(x = value, fill = variable), data = mserf) + 
  geom_density(alpha = .5)+
  ggtitle("Comparison of the Simulated MSEtest Distributions of CTree, Random, and Bagged Forests")+
  scale_fill_manual(values = thesis)+
  guides(fill=guide_legend(title=NULL))
