...
#       columns=c(1:7, 13), # columns to plot, default to all.
#       title=("Visualization of the Structure of Dataset D1")) 
...
To compare the two methods, lets compare the CART tree fitted on $D_2$ above with a recursive partitioned tree as implemented in the R package `rpart`. 

### Figure 10:

*CART and RPART fitted to $D_2$

```{r, message=FALSE, warning=FALSE, echo=FALSE}
t2.2 <- rpart(y~., round(d2))
t2.2$frame$yval <- round(t2.2$frame$yval)

par(mfrow=c(1,2))
draw.tree(t2.2, digits = 2, col = c(thesis, rep(thesis, 3)))
draw.tree(t2, digits = 2, col = c(thesis, rep(thesis, 3)))
```

Trees can be quite variable, so to get a better idea of the differences between the methods let's run a simulation.

**Psuedo-code simulation**
For $i \leq 1000$:
...
$$MSE_{test} = \frac 1 n (y_j - \hat{y_j})^2$$ Where $n$ is the number of observations in $D_{2, test}^i$, $y_j \in D_{2, test}^i, \hat{y_j} \in  T^i(D_{2, test}^i)$ for $1 \leq j \leq n$ 

This produces two distributions of $MSE_{test}$, one for CART and one for RPART. 
...
for(i in 1:1000){
  ...
  testmseR[i] <- mean((d2[-train,]$y - predict(tR, d2[-train,]))^2)
}

testmse <- data.frame(RPART = testmseR, CART= testmseC)

testm <- melt(testmse)

ggplot(data = testm, aes(fill = variable, x = value)) + 
  geom_density(alpha = .5)+
  ggtitle("Comparison of the Simulated MSEtest Distributions of RPART and CART")+
  scale_fill_manual(values = thesis)+
...
msebag <- data.frame(BaggedForest = mse, RPART = sample(testmseR,100))

msebag <- melt(msebag)

ggplot(aes(x = value, fill = variable), data = msebag) + 
  geom_density(alpha = .5)+
  ggtitle("Comparison of the Simulated MSEtest Distributions of RPART and Bagged Forests")+
  scale_fill_manual(values = thesis)+
  guides(fill=guide_legend(title=NULL))
...
mserf <- data.frame(BaggedForest = mse, RPART = sample(testmseR, 100), RandomForest = mseRF)

mserf<- melt(mserf)

ggplot(aes(x = value, fill = variable), data = mserf) + 
  geom_density(alpha = .5)+
  ggtitle("Comparison of the Simulated MSEtest Distributions of RPART, Random, and Bagged Forests")+
  scale_fill_manual(values = thesis)+
  guides(fill=guide_legend(title=NULL))
