...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Before we discuss surrogate splits, lets cover an obvious definition of variable importance of a single tree. In the tree represented by table \ref{tab:tabhomegrowntree}, define variable importance as the number of splits on each variable. This would allow us to answer the question: how useful (important) was variable $X_i$ in construbting our model for $Y$? Just by counting the splits on that variable, we would arrive at the following ranking: 
