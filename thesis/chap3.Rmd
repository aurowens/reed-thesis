...
As an example for $5$ in the algorithm above, consider the dataset $D_{3,lite}$, where $D_{3,lite} = {\omega_1, \omega_2, y}$ where $D_{3,lite}$ has dimensions $n = 100$, $p = 2$. A simple CART tree with 5 splits on the model $Y$ ~$\omega_1, \omega_2$
...
This thesis hopes to be a response to conditional variable importance as outlined by Strobl et al 2008. First is that the practice of permuting given the partitions from the model $Y \sim X_1,...,X_p$ instead of $X_j \sim X_1,..,X_p$ 
