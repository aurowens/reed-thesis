...
% Everything below added by CII
  ...
  importance. They are defined using logic that resembles the logic behind
  random forests.
  ...
  generate the same right daughter as \(N_{RR}\). Then the
  probability\textbf{EXPAND?}
  
  Basically a surrogate split \(\tilde{s}\),is one that estimates the best
  possible univariate split \(s*\) on node \(t\).
  
  \textbf{Defintion: Variable Importance, Single Tree}
  
  \[VI_{tree}(X_i) = \sum_{t \in T} \Delta RSS(\tilde{s_i}, t)\] Or the
  decrease of RSS attributable to \(X_i\) accross the tree \(T\).
