...
% Everything below added by CII
  ...
  \emph{Ripley, B. D. (1996) Pattern Recognition and Neural Networks.
  ...
  above with a recursive partitioned tree as implemented in the R package
  \texttt{rpart}.
  ...
  *CART and RPART fitted to \(D_2\)
  ...
  \textbf{Psuedo-code simulation} For \(i \leq 1000\): 1. Randomly sample
  \(\frac 2 3\) of the observations in \(D_2\) to a training set,
  \(D_{2, train}^i\). The other observations,
  \(x \in D_2, x \notin D_{2, train}^i\) form the testing set
  \(D_{2, test}^i\) 2. Fit a tree, \(T^i\), to the data under the model
  \(Y \sim X_1,...,X_2\) using the observations in \(D_{2}^i\) 3.
  Calculate the \(MSE_test\) of the model using the equation:
  \[MSE_{test} = \frac 1 n (y_j - \hat{y_j})^2\] Where \(n\) is the number
  of observations in \(D_{2, test}^i\),
  ...
  \subsection{Figure 10:}\label{figure-10-1}
  ...
  \begin{center}\includegraphics{Thesis_files/figure-latex/unnamed-chunk-14-1} \end{center}
