...
strobltrees <- function(t,y,xs) {
  ...
  for(i in 1:ncol(xs)){
    ...
    rss.post[i] <- (sum((y - as.numeric(predictions.for.y.xi.perm))^2)/length(y) - rss.pre)
    xs.for.permuting.i <- xs
  }
...
rf1 <- rforest(d1$y[1:200],d1[1:200, 1:12],mtry = 4,ntree = 100)

# rf2 <- rforest(d2$y[1:200],d2[1:200, 1:12],mtry = 4,ntree = 200)
...
# inf2 <- infforest(rf2, d2$y[1:200],d2[1:200, 1:12])
...
v$mtry <- rep(4, nrow(v))
set.seed(1)
r12 <- rforest(d1$y[1:200],d1[1:200, 1:12],mtry = 6,ntree = 100)
v12 <- as.data.frame(infforest(r12, d1$y[1:200],d1[1:200, 1:12]))
v12[v12 == "NaN"] <- NA
v12[v12 == "Inf"] <- NA
v12 <- v12[complete.cases(v12),]
set.seed(1)
r13 <- rforest(d1$y[1:200],d1[1:200, 1:12],mtry = 8,ntree = 100)
v13 <- as.data.frame(infforest(r13, d1$y[1:200],d1[1:200, 1:12]))
v13[v13 == "NaN"]<- NA
v13[v13 == "Inf"]<- NA
