...
####Table __: A Home-Grown Tree on $Y~X_1+X_2+X_3+X_4$

```{r, echo=FALSE, warning=FALSE, cache=TRUE}
 t1 <- tree.rf(d2$y[1:100],d2[1:100, 1:4],mtry = 4)
kable(t1[[2]])
```
...
```{r forest,echo=FALSE, message=FALSE, cache=TRUE}
rf1 <- rforest(d1$y[1:400],d1[1:400, 1:4],mtry = 2, ntree = 100)
rf2 <- rforest(d2$y[1:400],d2[1:400, 1:4],mtry = 2, ntree = 100)
rf3 <- rforest(d3$y[1:400],d3[1:400, 1:4],mtry = 2, ntree = 100)
```

```{r infforest,echo=FALSE, message=FALSE, cache=TRUE}
inf1 <- infforest(rf1, d1$y[1:400],d1[1:400, 1:4])
inf2 <- infforest(rf2, d2$y[1:400],d2[1:400, 1:4])
inf3 <- infforest(rf3, d3$y[1:400],d3[1:400, 1:4])
```

```{r infforestDist, echo=FALSE, message=FALSE, cache=TRUE, warning=FALSE}
inff1 <- melt(as.data.frame(inf1))
...
```{r figinfdists, echo = FALSE, message = FALSE, warning = FALSE, fig.cap="INFFOREST Distributions for Datasets D1, D2, and D3", results="asis"}

par(mfrow=c(1,3))
ggplot(data = inff2,aes(x = inff1$value, fill = inff1$var)) + 
  geom_density(alpha = .7) + 
  scale_fill_manual(values = thesis)+
  ggtitle("D1")
ggplot(data = inff2,aes(x = inff2$value, fill = inff2$var)) + 
  geom_density(alpha = .7)+ scale_fill_manual(values = thesis)+
  ggtitle("D2")
ggplot(data = inff2,aes(x = inff3$value, fill = inff3$var)) + 
  geom_density(alpha = .7)+ scale_fill_manual(values = thesis)+
  ggtitle("D3")
...
```{r x1x2infcomp, echo=FALSE, warning=FALSE, error=FALSE}
x21 <- inf3[inf3[,2] >= .5, c(1,2,3)]
x31 <- inf3[inf3[,2] < .5, c(1,2,3)]

x21melt <- melt(as.data.frame(x21[,-2]))
x31melt <- melt(as.data.frame(x31[,-2]))
```


```{r figx2x1inf, echo = FALSE, message = FALSE, warning = FALSE, fig.cap="INFFOREST Distributions for W2 and W3 when the INFFOREST value for X2 is >= .5 or Not ", results="asis"}
layout(matrix(c(1,2), 2, 2, byrow = TRUE))
ggplot(data = x21melt,aes(x = x21melt$value, fill = x21melt$var)) + 
  geom_density(alpha = .7)+ scale_fill_manual(values = thesis)
ggplot(data = x31melt,aes(x = x31melt$value, fill = x31melt$var)) + 
  geom_density(alpha = .7)+ scale_fill_manual(values = thesis)
```
